...
class LanguagesTableViewCell: UITableViewCell {
    ...
    func configureLanguageCell(language : String) {
        ...
        getPreferedLanguages { (languages) in
            if  languages.contains(self.languagesDic[language]!) {
                
                self.checkButton.setImage(UIImage(named:"heartFB.png"), for: .normal)
